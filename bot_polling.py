# =========================================================================
# ===            АБСОЛЮТНО ФИНАЛЬНАЯ ВЕРСИЯ BOT_POLLING.PY             ===
# =========================================================================
...
# ИСПРАВЛЕНИЕ 2: Список "мусорных" слов для очистки материала
...
def extract_entities(text: str) -> Tuple[str | None, str | None, str | None]:
    ...
    # ИСПРАВЛЕНИЕ 2: Улучшенная очистка материала от мусорных слов
...
def get_gigachat_answer(question: str) -> str:
    ...
    # ИСПРАВЛЕНИЕ 1: Очень строгий промпт для краткости
    system_prompt = (
        "Твоя роль - эксперт по экологии. Отвечай на вопросы, связанные с экологией и переработкой. "
        "Твой ответ должен быть ОДНИМ, максимум двумя предложениями. Ни при каких условиях не превышай лимит в 15 слов. "
        "Категорически запрещено отвечать списками, перечислениями или поэтапными инструкциями. "
        "Если вопрос не по теме, твой стандартный отказ: 'К сожалению, я могу обсуждать только вопросы, связанные с экологией.'"
    )
...
def handle_text(message):
    try:
        ...
        response = "" # Инициализируем переменную, чтобы избежать ошибок
        ...
        if city or (is_search_query and material):
            ...
            if answer:
                ...
                if context_to_save:
                    ...
                    markup.add(types.InlineKeyboardButton(text=button_text, callback_data=f"search_context_{context_to_save}"))
...
if __name__ == "__main__":
    ...
    except Exception as e: print(f"Бот остановился из-за ошибки: {e}")